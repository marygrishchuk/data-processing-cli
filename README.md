...
It is a **Data Processing Toolkit** — an interactive command-line application that performs various useful data processing operations. The tool works as a persistent Node.js process that accepts commands.

It's a real, cohesive tool with interactive file system navigation and data processing capabilities.

## Technical requirements

- Use 24.x.x version (24.10.0 or upper) of Node.js to run the app.
...
- Displays a welcome message on startup: `Welcome to Data Processing CLI!`
- Prints the current working directory initially: `You are currently in /path/to/home`
- Continuously prompts the user to enter commands: `>`
- Accepts commands in the format: `<command> [arguments]`
- Displays error messages for unknown or invalid commands without crashing
- Allows users to exit with `.exit` command or `Ctrl+C`
- Displays a goodbye message on exit: `Thank you for using Data Processing CLI!`
- After each successful operation, prints the current working directory again
- At the start of the program, working directory is the user's home directory
